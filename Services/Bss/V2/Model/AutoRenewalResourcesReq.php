...
class AutoRenewalResourcesReq implements ModelInterface, ArrayAccess
{
    ...
    * periodType  |参数名称：自动续费的周期类型| |参数的约束及描述：该参数非必填，自动续费的周期类型，支持枚举| |MONTH：包月，YEAR：包年。此参数不携带或携带值为null时，按照如下规则处理。购买时未设置自动续费功能，默认与设置资源最后一个订单的订购周期类型一致。|
    *
    * @var string[]
    */
    ...
    * Sets periodType
    *
    * @param string|null $periodType |参数名称：自动续费的周期类型| |参数的约束及描述：该参数非必填，自动续费的周期类型，支持枚举| |MONTH：包月，YEAR：包年。此参数不携带或携带值为null时，按照如下规则处理。购买时未设置自动续费功能，默认与设置资源最后一个订单的订购周期类型一致。|
    *
    * @return $this
    */
