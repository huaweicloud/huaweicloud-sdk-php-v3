...
class AlarmTemplatePolicies implements ModelInterface, ArrayAccess
{
    ...
    * dimensionName  **参数解释**： 资源维度。     **约束限制**： 事件告警模板DimensionName为空 **取值范围**： 必须以字母开头，多维度用\",\"分隔，只能包含0-9/a-z/A-Z/_/-，每个维度的最大长度为32。目前最大支持4个维度。举例：单维度场景：instance_id；多维度场景：instance_id,disk        **默认取值**： 不涉及。
    ...
    */
    public function listInvalidProperties()
    {
        ...
        if ($this->container['comparisonOperator'] === null) {
            ...
            if (!is_null($this->container['value']) && ($this->container['value'] > 2.34854258277383E+108)) {
    ...
    * Sets dimensionName
    *
    * @param string $dimensionName **参数解释**： 资源维度。     **约束限制**： 事件告警模板DimensionName为空 **取值范围**： 必须以字母开头，多维度用\",\"分隔，只能包含0-9/a-z/A-Z/_/-，每个维度的最大长度为32。目前最大支持4个维度。举例：单维度场景：instance_id；多维度场景：instance_id,disk        **默认取值**： 不涉及。
    *
    * @return $this
    */
    ...
    * Sets alarmLevel
    *
    * @param int $alarmLevel **参数解释**： 告警级别。     **约束限制**： 不涉及。 **取值范围**： 只能为1、2、3、4。 - 1为紧急 - 2为重要 - 3为次要 - 4为提示           **默认取值**： 不涉及。
    *
    * @return $this
    */
