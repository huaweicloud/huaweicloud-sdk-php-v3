...
class AlarmHistoryItemV2Condition implements ModelInterface, ArrayAccess
{
    ...
    * period  **参数解释**： 指标周期，单位是秒。如想了解各个云服务的指标原始周期可以参考“[支持服务列表](ces_03_0059.xml)”。 **取值范围**： 0是默认值，例如事件类告警该字段就用0即可； 1代表指标的原始周期，比如RDS监控指标原始周期是60s，表示该RDS指标按60s周期为一个数据点参与告警计算； 300代表指标按5分钟聚合周期为一个数据点参与告警计算； 1200代表指标按20分钟聚合周期为一个数据点参与告警计算； 3600代表指标按60分钟聚合周期为一个数据点参与告警计算； 14400代表指标按4小时聚合周期为一个数据点参与告警计算； 86400代表指标按1天聚合周期为一个数据点参与告警计算。
    * filter  **参数解释**： 聚合方式。 **取值范围**： 枚举值。average：平均值，min：最小值，max：最大值，sum：求和，tp99：99百分位数，tp95：95百分位数，tp90：90百分位数。字符长度在 1 到 15之间。
    * comparisonOperator  **参数解释**： 阈值符号。 **取值范围**： 枚举值。支持的值为(>|<|>=|<=|=|!=|cycle_decrease|cycle_increase|cycle_wave);cycle_decrease为环比下降,cycle_increase为环比上升,cycle_wave为环比波动。字符长度在 1 到 10之间。
    * value  **参数解释**： 告警阈值。 **取值范围**： 具体阈值取值请参见附录中各服务监控指标中取值范围，如[支持监控的服务列表](ces_03_0059.xml)中ECS的CPU使用率cpu_util取值范围可配置80。最小值为0，最大值为1.7976931348623157e+108。
    * unit  **参数解释**： 数据的单位。 **取值范围**： 字符串长度最大为 32。
    * count  **参数解释**： 告警连续触发次数。 **取值范围**： 字符串长度在 1 到 180 之间。
    * suppressDuration  **参数解释**： 告警抑制时间（告警周期），单位为秒，对应页面上创建告警规则时告警策略最后一个字段，该字段主要为解决告警频繁的问题。 **取值范围**： 0代表不抑制，满足条件即告警； 300代表满足告警触发条件后每5分钟告警一次； 600代表满足告警触发条件后每10分钟告警一次； 900代表满足告警触发条件后每15分钟告警一次； 1800代表满足告警触发条件后每30分钟告警一次； 3600代表满足告警触发条件后每60分钟告警一次； 10800代表满足告警触发条件后每3小时告警一次； 21600代表满足告警触发条件后每6小时告警一次； 43200代表满足告警触发条件后每12小时告警一次； 8600代表满足告警触发条件后每一天告警一次；
    *
    * @var string[]
    */
    ...
    */
    public function listInvalidProperties()
    {
        $invalidProperties = [];
            ...
            if (!is_null($this->container['filter']) && !preg_match("/^(average|min|max|sum|tp99|tp95|tp90)$/", $this->container['filter'])) {
                $invalidProperties[] = "invalid value for 'filter', must be conform to the pattern /^(average|min|max|sum|tp99|tp95|tp90)$/.";
            }
    ...
    * Sets period
    *
    * @param int|null $period **参数解释**： 指标周期，单位是秒。如想了解各个云服务的指标原始周期可以参考“[支持服务列表](ces_03_0059.xml)”。 **取值范围**： 0是默认值，例如事件类告警该字段就用0即可； 1代表指标的原始周期，比如RDS监控指标原始周期是60s，表示该RDS指标按60s周期为一个数据点参与告警计算； 300代表指标按5分钟聚合周期为一个数据点参与告警计算； 1200代表指标按20分钟聚合周期为一个数据点参与告警计算； 3600代表指标按60分钟聚合周期为一个数据点参与告警计算； 14400代表指标按4小时聚合周期为一个数据点参与告警计算； 86400代表指标按1天聚合周期为一个数据点参与告警计算。
    *
    * @return $this
    */
    ...
    * Sets filter
    *
    * @param string|null $filter **参数解释**： 聚合方式。 **取值范围**： 枚举值。average：平均值，min：最小值，max：最大值，sum：求和，tp99：99百分位数，tp95：95百分位数，tp90：90百分位数。字符长度在 1 到 15之间。
    *
    * @return $this
    */
    ...
    *
    * @param string|null $comparisonOperator **参数解释**： 阈值符号。 **取值范围**： 枚举值。支持的值为(>|<|>=|<=|=|!=|cycle_decrease|cycle_increase|cycle_wave);cycle_decrease为环比下降,cycle_increase为环比上升,cycle_wave为环比波动。字符长度在 1 到 10之间。
    *
    * @return $this
    */
    ...
    * Sets value
    *
    * @param double|null $value **参数解释**： 告警阈值。 **取值范围**： 具体阈值取值请参见附录中各服务监控指标中取值范围，如[支持监控的服务列表](ces_03_0059.xml)中ECS的CPU使用率cpu_util取值范围可配置80。最小值为0，最大值为1.7976931348623157e+108。
    *
    * @return $this
    */
    ...
    * Sets count
    *
    * @param int|null $count **参数解释**： 告警连续触发次数。 **取值范围**： 字符串长度在 1 到 180 之间。
    *
    * @return $this
    */
    ...
    *
    * @param int|null $suppressDuration **参数解释**： 告警抑制时间（告警周期），单位为秒，对应页面上创建告警规则时告警策略最后一个字段，该字段主要为解决告警频繁的问题。 **取值范围**： 0代表不抑制，满足条件即告警； 300代表满足告警触发条件后每5分钟告警一次； 600代表满足告警触发条件后每10分钟告警一次； 900代表满足告警触发条件后每15分钟告警一次； 1800代表满足告警触发条件后每30分钟告警一次； 3600代表满足告警触发条件后每60分钟告警一次； 10800代表满足告警触发条件后每3小时告警一次； 21600代表满足告警触发条件后每6小时告警一次； 43200代表满足告警触发条件后每12小时告警一次； 8600代表满足告警触发条件后每一天告警一次；
    *
    * @return $this
    */
