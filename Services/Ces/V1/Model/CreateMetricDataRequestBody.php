...
class CreateMetricDataRequestBody implements ModelInterface, ArrayAccess
{
    ...
    * @var string
    */
    protected static $openAPIModelName = 'CreateMetricDataRequestBody';

    /**
    * Array of property to type mappings. Used for (de)serialization
    * metric  metric
    * ttl  **参数解释**： 数据的有效期，超出该有效期则自动删除该数据，单位秒。 **约束限制**： 不涉及。 **取值范围**： 大小为[1,604800]的整数。 **默认取值**： 不涉及。
    * collectTime  **参数解释**： 数据收集时间 。UNIX时间戳，单位毫秒。 **约束限制**： 不涉及。 **取值范围**： 因为客户端到服务器端有延时，因此插入数据的时间戳应该在[当前时间-3天+10秒，当前时间+10分钟-10秒]区间内，保证到达服务器时不会因为传输时延造成数据不能插入数据库。 **默认取值**： 不涉及。
    * value  **参数解释**： 待添加的监控指标数据的值。 **约束限制**： 不涉及。 **取值范围**： 数值类型支持“整数”或“浮点数”。 **默认取值**： 不涉及。
    * unit  **参数解释**： 数据的单位。 **约束限制**： 不涉及。 **取值范围**： 长度为[0,32]个字符。 **默认取值**： 不涉及。
    * type  **参数解释**： 数据类型。 **约束限制**： 不涉及。 **取值范围**： 枚举值，只能是\"int\"或\"float\"。 - int：整数 - float：浮点数 **默认取值**： 不涉及。
    *
    * @var string[]
    */
    ...
    */
    public function listInvalidProperties()
    {
        ...
        if ($this->container['collectTime'] === null) {
            ...
            if (($this->container['collectTime'] > 9999999999999)) {
                $invalidProperties[] = "invalid value for 'collectTime', must be smaller than or equal to 9999999999999.";
            }
            if (($this->container['collectTime'] < 1111111111111)) {
                $invalidProperties[] = "invalid value for 'collectTime', must be bigger than or equal to 1111111111111.";
            }
        if ($this->container['value'] === null) {
            ...
            if (!is_null($this->container['type']) && !preg_match("/^(int|float)$/", $this->container['type'])) {
                $invalidProperties[] = "invalid value for 'type', must be conform to the pattern /^(int|float)$/.";
            }
    ...
    * Sets ttl
    *
    * @param int $ttl **参数解释**： 数据的有效期，超出该有效期则自动删除该数据，单位秒。 **约束限制**： 不涉及。 **取值范围**： 大小为[1,604800]的整数。 **默认取值**： 不涉及。
    *
    * @return $this
    */
    ...
    * Sets collectTime
    *
    * @param int $collectTime **参数解释**： 数据收集时间 。UNIX时间戳，单位毫秒。 **约束限制**： 不涉及。 **取值范围**： 因为客户端到服务器端有延时，因此插入数据的时间戳应该在[当前时间-3天+10秒，当前时间+10分钟-10秒]区间内，保证到达服务器时不会因为传输时延造成数据不能插入数据库。 **默认取值**： 不涉及。
    *
    * @return $this
    */
    ...
    * Sets value
    *
    * @param double $value **参数解释**： 待添加的监控指标数据的值。 **约束限制**： 不涉及。 **取值范围**： 数值类型支持“整数”或“浮点数”。 **默认取值**： 不涉及。
    *
    * @return $this
    */
    ...
    * Sets type
    *
    * @param string|null $type **参数解释**： 数据类型。 **约束限制**： 不涉及。 **取值范围**： 枚举值，只能是\"int\"或\"float\"。 - int：整数 - float：浮点数 **默认取值**： 不涉及。
    *
    * @return $this
    */
