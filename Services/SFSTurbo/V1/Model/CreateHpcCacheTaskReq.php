...
class CreateHpcCacheTaskReq implements ModelInterface, ArrayAccess
{
    ...
    * srcTarget  联动目录名称
    * srcPrefix  导入导出任务的源端路径前缀，导入时不需要包含OBS桶名，导出时不需要包含联动目录名称。  当 type 为 import、import_metadata、preload 时，该参数必须是 OBS后端存储内的相对路径前缀。 当 type 为 export 时，该参数必须是文件系统内file_system_path下的相对路径前缀。  如果不带该字段，导入时会导入绑定OBS桶内的所有对象，导出时会导出联动目录下的所有文件。限制如下： - 长度为1~1024个字符之间 - 不能包含以下字符：\\:*?\"<>| - 不能以英文句号.开头和结尾
    ...
    * Sets srcPrefix
    *
    * @param string|null $srcPrefix 导入导出任务的源端路径前缀，导入时不需要包含OBS桶名，导出时不需要包含联动目录名称。  当 type 为 import、import_metadata、preload 时，该参数必须是 OBS后端存储内的相对路径前缀。 当 type 为 export 时，该参数必须是文件系统内file_system_path下的相对路径前缀。  如果不带该字段，导入时会导入绑定OBS桶内的所有对象，导出时会导出联动目录下的所有文件。限制如下： - 长度为1~1024个字符之间 - 不能包含以下字符：\\:*?\"<>| - 不能以英文句号.开头和结尾
    *
    * @return $this
    */
