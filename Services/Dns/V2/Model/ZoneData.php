...
class ZoneData implements ModelInterface, ArrayAccess
{
    ...
    * description  **参数解释：** 对域名的描述信息。 **取值范围：** 长度不超过255个字符。
    ...
    * status  **参数解释：** 域名状态。 **取值范围：** - ACTIVE：正常 - PENDING_CREATE：创建中 - PENDING_UPDATE：更新中 - PENDING_DELETE：删除中 - PENDING_FREEZE：冻结中 - FREEZE：冻结 - ILLEGAL：违规冻结 - POLICE：公安冻结 - PENDING_DISABLE：暂停中 - DISABLE：暂停 - ERROR：失败
    * poolId  **参数解释：** 托管该域名的pool，由系统分配。 **取值范围：** 不涉及。
    * projectId  **参数解释：** 域名所属的项目ID。 **取值范围：** 不涉及。
    * zoneType  **参数解释：** 域名类型。 **取值范围：** - public：公网域名 - private：内网域名
    ...
    * Sets description
    *
    * @param string|null $description **参数解释：** 对域名的描述信息。 **取值范围：** 长度不超过255个字符。
    *
    * @return $this
    */
    ...
    * Sets status
    *
    * @param string|null $status **参数解释：** 域名状态。 **取值范围：** - ACTIVE：正常 - PENDING_CREATE：创建中 - PENDING_UPDATE：更新中 - PENDING_DELETE：删除中 - PENDING_FREEZE：冻结中 - FREEZE：冻结 - ILLEGAL：违规冻结 - POLICE：公安冻结 - PENDING_DISABLE：暂停中 - DISABLE：暂停 - ERROR：失败
    *
    * @return $this
    */
    ...
    * Sets zoneType
    *
    * @param string|null $zoneType **参数解释：** 域名类型。 **取值范围：** - public：公网域名 - private：内网域名
    *
    * @return $this
    */
