...
class GroupConfigUpdateRequest implements ModelInterface, ArrayAccess
{
    ...
    * name  **参数解释：** 部署名称。 **约束限制：** 不填保留原有值。 **取值范围：** 不涉及。 **默认取值：** 不涉及。
    * poolId  **参数解释：** 资源池ID，查询指定资源池下的服务，默认不过滤。可通过[查询资源池列表](ShowPool.xml)获取。 **约束限制：** 不填保留原有值。 **取值范围：** 不涉及。 **默认取值：** 不涉及。
    * framework  **参数解释：** 框架类型。 **约束限制：** 不填保留原有值。 **取值范围：** - COMMON：普通在线服务 - VLLM：VLLM框架 - MINDIE：MINDIE框架 **默认取值：** 不涉及。
    * count  **参数解释：** 部署场景下，服务实例数量。 **约束限制：** 不填保留原有值。 **取值范围：** [1, 128]。 **默认取值：** 不涉及。
    * deployType  **参数解释：** 部署类型。 **约束限制：** 不填保留原有值。 **取值范围：** - SINGLE：常规部署 - MULTI：分布式部署 **默认取值：** 不涉及
    * systemLogDumpEnable  **参数解释：** 系统日志转储开关。 **约束限制：** 不填保留原有值。 **取值范围：** 不涉及 **默认取值：** 不涉及。
    * unitConfigs  **参数解释：** 实例单元配置。 **约束限制：** - 单机推理时，个数只会为1；如果是分布式推理时，根据不同框架，实例单元配置可灵活配置。 - 不填保留原有值，填了全量覆盖原有值。 **取值范围：** 不涉及 **默认取值：** 不涉及
    ...
    * highAvailSwitch  **参数解释：** 高可用开关，开启后不同实例的pod将尽量均匀分布到不同的节点上。 **约束限制：** 不填保留原有值 **取值范围：** 不涉及 **默认取值：** 不涉及
    * description  **参数解释：** 部署备注。 **约束限制：** 不填保留原有值。 **取值范围：** 长度不可以超过512，不能包含大于号，小于号。 **默认取值：** 默认为空。
    ...
    * scheduleStrategy  **参数解释：** 调度策略。 **约束限制：** 不填保留原有值。 **取值范围：** - HIGH_AVAILABILITY：高可用调度 - HIGH_UTILIZATION：紧凑调度 - HIGH_PERFORMANCE：高性能调度 **默认取值：** HIGH_AVAILABILITY。
    *
    * @var string[]
    */
    ...
    * Sets name
    *
    * @param string $name **参数解释：** 部署名称。 **约束限制：** 不填保留原有值。 **取值范围：** 不涉及。 **默认取值：** 不涉及。
    *
    * @return $this
    */
    ...
    * Sets framework
    *
    * @param string|null $framework **参数解释：** 框架类型。 **约束限制：** 不填保留原有值。 **取值范围：** - COMMON：普通在线服务 - VLLM：VLLM框架 - MINDIE：MINDIE框架 **默认取值：** 不涉及。
    *
    * @return $this
    */
    ...
    * Sets count
    *
    * @param int|null $count **参数解释：** 部署场景下，服务实例数量。 **约束限制：** 不填保留原有值。 **取值范围：** [1, 128]。 **默认取值：** 不涉及。
    *
    * @return $this
    */
    ...
    *
    * @param bool|null $systemLogDumpEnable **参数解释：** 系统日志转储开关。 **约束限制：** 不填保留原有值。 **取值范围：** 不涉及 **默认取值：** 不涉及。
    *
    * @return $this
    */
    ...
    * Sets unitConfigs
    *
    * @param \HuaweiCloud\SDK\ModelArts\V1\Model\UnitConfig[]|null $unitConfigs **参数解释：** 实例单元配置。 **约束限制：** - 单机推理时，个数只会为1；如果是分布式推理时，根据不同框架，实例单元配置可灵活配置。 - 不填保留原有值，填了全量覆盖原有值。 **取值范围：** 不涉及 **默认取值：** 不涉及
    *
    * @return $this
    */
    ...
    *
    * @param bool|null $highAvailSwitch **参数解释：** 高可用开关，开启后不同实例的pod将尽量均匀分布到不同的节点上。 **约束限制：** 不填保留原有值 **取值范围：** 不涉及 **默认取值：** 不涉及
    *
    * @return $this
    */
    ...
    * Sets description
    *
    * @param string|null $description **参数解释：** 部署备注。 **约束限制：** 不填保留原有值。 **取值范围：** 长度不可以超过512，不能包含大于号，小于号。 **默认取值：** 默认为空。
    *
    * @return $this
    */
    ...
    *
    * @param string|null $scheduleStrategy **参数解释：** 调度策略。 **约束限制：** 不填保留原有值。 **取值范围：** - HIGH_AVAILABILITY：高可用调度 - HIGH_UTILIZATION：紧凑调度 - HIGH_PERFORMANCE：高性能调度 **默认取值：** HIGH_AVAILABILITY。
    *
    * @return $this
    */
