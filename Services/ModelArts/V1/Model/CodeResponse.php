...
class CodeResponse implements ModelInterface, ArrayAccess
{
    ...
    * source  **参数解释：** 代码来源类别。 **取值范围：** - OBS：对象存储服务。 - OBSFS：OBS的文件系统接口。 - EFS：弹性文件服务。
    * address  **参数解释：** 代码来源地址，格式遵循不同存储系统。 **取值范围：** 不涉及。
    * sourceId  **参数解释：** 代码来源ID，与address二选一，当且仅当source为EFS时，可以传入sfs turbo的ID。 **取值范围：** 不涉及。
    * mountPath  **参数解释：** 挂载到容器内的路径。 **约束限制：** 不涉及。 **取值范围：** 以(/)开头和结尾，可包含字母、数字、中划线、下划线，整个挂载路径长度不能超过255位。 **默认取值：** 不涉及。
    * efsSubPath  **参数解释：** EFS子路径。 **取值范围：** 不涉及。
    ...
    * Sets source
    *
    * @param string $source **参数解释：** 代码来源类别。 **取值范围：** - OBS：对象存储服务。 - OBSFS：OBS的文件系统接口。 - EFS：弹性文件服务。
    *
    * @return $this
    */
    ...
    * Sets sourceId
    *
    * @param string|null $sourceId **参数解释：** 代码来源ID，与address二选一，当且仅当source为EFS时，可以传入sfs turbo的ID。 **取值范围：** 不涉及。
    *
    * @return $this
    */
    ...
    * Sets efsSubPath
    *
    * @param string|null $efsSubPath **参数解释：** EFS子路径。 **取值范围：** 不涉及。
    *
    * @return $this
    */
