...
class ListServerlessAssetDetailRequest implements ModelInterface, ArrayAccess
{
    ...
    * @var string
    */
    protected static $openAPIModelName = 'ListServerlessAssetDetailRequest';

    /**
    * Array of property to type mappings. Used for (de)serialization
    * enterpriseProjectId  **参数解释**: 企业项目ID，用于过滤不同企业项目下的资产。获取方式请参见[获取企业项目ID](hss_02_0027.xml)。 如需查询所有企业项目下的资产请传参“all_granted_eps”。 **约束限制**: 开通企业项目功能后才需要配置企业项目ID参数。 **取值范围**: 字符长度1-256位 **默认取值**: 0，表示默认企业项目（default）。
    * category  **参数解释**： 资产维度 **约束限制**： 不涉及 **取值范围**： - container : 容器维度 **默认取值**： container
    * assetId  **参数解释**: 资产ID，基于参数category决定维度（比如category=container时，asset_id为容器id） **约束限制**： 不涉及 **取值范围**: 字符长度0-255位 **默认取值**： 不涉及
    *
    * @var string[]
    */
    ...
    */
    public function listInvalidProperties()
    {
        ...
        if ($this->container['category'] === null) {
            ...
            if (!preg_match("/^(container)$/", $this->container['category'])) {
                $invalidProperties[] = "invalid value for 'category', must be conform to the pattern /^(container)$/.";
            }
        if ($this->container['assetId'] === null) {
            $invalidProperties[] = "'assetId' can't be null";
        }
            if ((mb_strlen($this->container['assetId']) > 255)) {
                $invalidProperties[] = "invalid value for 'assetId', the character length must be smaller than or equal to 255.";
            }
            if ((mb_strlen($this->container['assetId']) < 0)) {
