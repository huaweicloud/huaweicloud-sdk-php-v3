...
class ProtectionServerInfoBackupError implements ModelInterface, ArrayAccess
{
    ...
    * errorCode  **参数解释**: 错误编码 **取值范围**: 包含如下4种。   - 0 ：无错误信息。   - 1 ：已绑定至其它存储库，无法开启备份。   - 2 ：备份库已超过最大限额。   - 3 ：CBR接口调用异常。
    ...
    * Sets errorCode
    *
    * @param int|null $errorCode **参数解释**: 错误编码 **取值范围**: 包含如下4种。   - 0 ：无错误信息。   - 1 ：已绑定至其它存储库，无法开启备份。   - 2 ：备份库已超过最大限额。   - 3 ：CBR接口调用异常。
    *
    * @return $this
    */
