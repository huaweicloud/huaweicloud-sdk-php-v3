...
class ShowImageUploadCommandRequest implements ModelInterface, ArrayAccess
{
    ...
    * @var string
    */
    protected static $openAPIModelName = 'ShowImageUploadCommandRequest';

    /**
    * Array of property to type mappings. Used for (de)serialization
    * registryAddr  **参数解释**： 镜像仓地址 **约束限制**： 网址/IP:端口。如：myharbor.com。 **取值范围**： 字符长度1-256位  **默认取值**： 不涉及
    * namespace  **参数解释**： 镜像仓项目,用来指定扫描组件要上传到的镜像仓目录。 **约束限制**： 不涉及 **取值范围**： 字符长度1-128位  **默认取值**： 不涉及
    * username  **参数解释**： 用于登录镜像仓的用户名。 **约束限制**： 不涉及 **取值范围**： 字符长度1-128位  **默认取值**： 不涉及
    * password  **参数解释**： 用于登录镜像仓的密码。仅用于生成命令，HSS服务不会存储您的镜像仓密码。 **约束限制**： 不涉及 **取值范围**： 字符长度1-128位  **默认取值**： 不涉及
    *
    * @var string[]
    */
    ...
    */
    public function listInvalidProperties()
    {
        ...
        if ($this->container['namespace'] === null) {
            ...
            if ((mb_strlen($this->container['namespace']) > 128)) {
            ...
            if (!preg_match("/^.*$/", $this->container['namespace'])) {
