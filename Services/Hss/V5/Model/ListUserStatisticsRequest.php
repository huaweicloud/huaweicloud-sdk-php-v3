...
class ListUserStatisticsRequest implements ModelInterface, ArrayAccess
{
    ...
    * userName  **参数解释**: 用于筛选指定名称的账号统计数据（支持模糊匹配） **约束限制**: 符合Windows文件命名规则，不包含中文标点符号 **取值范围**: 字符长度1-64位 **默认取值**: 不涉及
    ...
    * Sets userName
    *
    * @param string|null $userName **参数解释**: 用于筛选指定名称的账号统计数据（支持模糊匹配） **约束限制**: 符合Windows文件命名规则，不包含中文标点符号 **取值范围**: 字符长度1-64位 **默认取值**: 不涉及
    *
    * @return $this
    */
