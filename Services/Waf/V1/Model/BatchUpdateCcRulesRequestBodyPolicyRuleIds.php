...
class BatchUpdateCcRulesRequestBodyPolicyRuleIds implements ModelInterface, ArrayAccess
{
    ...
    * @var string
    */
    protected static $openAPIModelName = 'BatchUpdateCcRulesRequestBody_policy_rule_ids';

    /**
    * Array of property to type mappings. Used for (de)serialization
    * policyId  **参数解释：** 策略id，唯一标识一条防护策略，可从\"查询防护策略列表\"(ListPolicy)接口获取 **约束限制：** 不涉及 **取值范围：** 不涉及 **默认取值：** 不涉及
    * ruleIds  **参数解释：** 规则id数组，包含当前防护策略下的单个CC规则id，您可以通过调用查询CC防护规则列表（ListCcRules）获取CC规则id **约束限制：** 单条规则ID **取值范围：** 不涉及 **默认取值：** 不涉及
    *
    * @var string[]
    */
    ...
    * Sets ruleIds
    *
    * @param string[] $ruleIds **参数解释：** 规则id数组，包含当前防护策略下的单个CC规则id，您可以通过调用查询CC防护规则列表（ListCcRules）获取CC规则id **约束限制：** 单条规则ID **取值范围：** 不涉及 **默认取值：** 不涉及
    *
    * @return $this
    */
