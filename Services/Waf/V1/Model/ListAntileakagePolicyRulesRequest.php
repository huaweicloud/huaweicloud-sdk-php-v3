...
class ListAntileakagePolicyRulesRequest implements ModelInterface, ArrayAccess
{
    ...
    * policyids  **参数解释：** 防护策略id，您可以通过调用查询防护策略列表（ListPolicy）获取策略id；设置此参数后可查询指定策略下的规则，可以设置多个 **约束限制：** 不涉及 **取值范围：** 不涉及 **默认取值：** 不涉及
    ...
    * Sets policyids
    *
    * @param string|null $policyids **参数解释：** 防护策略id，您可以通过调用查询防护策略列表（ListPolicy）获取策略id；设置此参数后可查询指定策略下的规则，可以设置多个 **约束限制：** 不涉及 **取值范围：** 不涉及 **默认取值：** 不涉及
    *
    * @return $this
    */
