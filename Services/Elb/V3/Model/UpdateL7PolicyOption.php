...
class UpdateL7PolicyOption implements ModelInterface, ArrayAccess
{
    ...
    * redirectPoolId  转发到pool的ID。当action为REDIRECT_TO_POOL时生效。  使用说明： - redirect_pool_id和redirect_pools_config两个都指定时按redirect_pools_config生效。 - 当action为REDIRECT_TO_POOL时为必选字段，不能更新为空或null。 - 指定的pool不能是listener的default_pool。不能是其他listener的l7policy使用的pool。 - 当action为REDIRECT_TO_LISTENER时，不可指定。
    ...
    */
    ...
    /**
    * Gets redirectPoolId
    *  转发到pool的ID。当action为REDIRECT_TO_POOL时生效。  使用说明： - redirect_pool_id和redirect_pools_config两个都指定时按redirect_pools_config生效。 - 当action为REDIRECT_TO_POOL时为必选字段，不能更新为空或null。 - 指定的pool不能是listener的default_pool。不能是其他listener的l7policy使用的pool。 - 当action为REDIRECT_TO_LISTENER时，不可指定。
    *
    * @return string|null
    */
    ...
    * Sets redirectPoolId
    *
    * @param string|null $redirectPoolId 转发到pool的ID。当action为REDIRECT_TO_POOL时生效。  使用说明： - redirect_pool_id和redirect_pools_config两个都指定时按redirect_pools_config生效。 - 当action为REDIRECT_TO_POOL时为必选字段，不能更新为空或null。 - 指定的pool不能是listener的default_pool。不能是其他listener的l7policy使用的pool。 - 当action为REDIRECT_TO_LISTENER时，不可指定。
    *
    * @return $this
    */
