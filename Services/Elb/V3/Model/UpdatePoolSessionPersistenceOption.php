...
class UpdatePoolSessionPersistenceOption implements ModelInterface, ArrayAccess
{
    ...
    * persistenceTimeout  会话保持的时间。当type为APP_COOKIE时不生效。  适用范围：如果pool的protocol为TCP、UDP和QUIC则范围为[1,60]（分钟），默认值1； 如果pool的protocol为HTTP和HTTPS则范围为[1,1440]（分钟），默认值1440。  [不支持QUIC。](tag:hws_eu,g42,hk_g42,hcso_dt,tm)
    *
    * @var string[]
    */
    ...
    *
    * @param int|null $persistenceTimeout 会话保持的时间。当type为APP_COOKIE时不生效。  适用范围：如果pool的protocol为TCP、UDP和QUIC则范围为[1,60]（分钟），默认值1； 如果pool的protocol为HTTP和HTTPS则范围为[1,1440]（分钟），默认值1440。  [不支持QUIC。](tag:hws_eu,g42,hk_g42,hcso_dt,tm)
    *
    * @return $this
    */
