...
class RedirectPoolsConfig implements ModelInterface, ArrayAccess
{
    ...
    * poolId  **参数解释**：所在后端服务器组ID。  **取值范围**：不涉及
    * weight  **参数解释**：转发策略服务器组的权重。请求将根据该权重进行负载分发到不同的服务器组。权重值越大，分发的请求越多。权重为0的后端不再接受新的请求。  **取值范围**：0-100
    *
    * @var string[]
    */
    ...
    * Sets poolId
    *
    * @param string|null $poolId **参数解释**：所在后端服务器组ID。  **取值范围**：不涉及
    *
    * @return $this
    */
    ...
    * Sets weight
    *
    * @param int|null $weight **参数解释**：转发策略服务器组的权重。请求将根据该权重进行负载分发到不同的服务器组。权重值越大，分发的请求越多。权重为0的后端不再接受新的请求。  **取值范围**：0-100
    *
    * @return $this
    */
