...
class CreateL7PolicyOption implements ModelInterface, ArrayAccess
{
    ...
    * listenerId  参数解释：转发策略对应的监听器ID。  约束限制： - 当action为REDIRECT_TO_POOL时，只支持创建在PROTOCOL为HTTP或HTTPS的listener上。 - 当action为REDIRECT_TO_LISTENER时，只支持创建在PROTOCOL为HTTP的listener上。
    * name  参数解释：转发策略名称。
    * position  参数解释：转发策略的优先级。  约束限制：不支持更新。  不支持该字段，请勿使用。
    ...
    * redirectListenerId  参数解释：转发到的listener的ID，当action为REDIRECT_TO_LISTENER时必选。  约束限制： - 只支持protocol为HTTPS/TERMINATED_HTTPS的listener。 - 不能指定为其他loadbalancer下的listener。 - 当action为REDIRECT_TO_POOL时，创建或更新时不能传入该参数。 [- 共享型负载均衡器下的转发策略不支持该字段。](tag:hws,hws_hk,ocb,ctc,g42,tm,cmcc,hk_g42,hws_ocb,hk_vdf,fcs,dt,hk_tm)
    * redirectPoolId  参数解释：转发到pool的ID。  约束限制： - 当action为REDIRECT_TO_POOL时生效。 - 当action为REDIRECT_TO_LISTENER时，传入会报错。
    * redirectUrl  参数解释：转发到的url。  约束限制：必须满足格式: protocol://host:port/path?query。  [不支持该字段，请勿使用。](tag:hcso_dt)
    ...
    * Sets listenerId
    *
    * @param string $listenerId 参数解释：转发策略对应的监听器ID。  约束限制： - 当action为REDIRECT_TO_POOL时，只支持创建在PROTOCOL为HTTP或HTTPS的listener上。 - 当action为REDIRECT_TO_LISTENER时，只支持创建在PROTOCOL为HTTP的listener上。
    *
    * @return $this
    */
    ...
    * Sets position
    *
    * @param int|null $position 参数解释：转发策略的优先级。  约束限制：不支持更新。  不支持该字段，请勿使用。
    *
    * @return $this
    */
    ...
    * Sets projectId
    *
    * @param string|null $projectId 参数解释：转发策略所在的项目ID。
    *
    * @return $this
    */
    ...
    *
    * @param string|null $redirectListenerId 参数解释：转发到的listener的ID，当action为REDIRECT_TO_LISTENER时必选。  约束限制： - 只支持protocol为HTTPS/TERMINATED_HTTPS的listener。 - 不能指定为其他loadbalancer下的listener。 - 当action为REDIRECT_TO_POOL时，创建或更新时不能传入该参数。 [- 共享型负载均衡器下的转发策略不支持该字段。](tag:hws,hws_hk,ocb,ctc,g42,tm,cmcc,hk_g42,hws_ocb,hk_vdf,fcs,dt,hk_tm)
    *
    * @return $this
    */
    ...
    * Sets redirectPoolId
    *
    * @param string|null $redirectPoolId 参数解释：转发到pool的ID。  约束限制： - 当action为REDIRECT_TO_POOL时生效。 - 当action为REDIRECT_TO_LISTENER时，传入会报错。
    *
    * @return $this
    */
    ...
    * Sets redirectUrl
    *
    * @param string|null $redirectUrl 参数解释：转发到的url。  约束限制：必须满足格式: protocol://host:port/path?query。  [不支持该字段，请勿使用。](tag:hcso_dt)
    *
    * @return $this
    */
    ...
    *
    * @param \HuaweiCloud\SDK\Elb\V3\Model\CreateRedirectPoolsConfig[]|null $redirectPoolsConfig 参数解释：转发到多个主机组列表。  约束限制：一个policy最多配置5个pool。
    *
    * @return $this
    */
