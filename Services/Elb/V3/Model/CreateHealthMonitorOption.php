...
class CreateHealthMonitorOption implements ModelInterface, ArrayAccess
{
    ...
    * monitorPort  健康检查端口号。取值：1-65535，默认为空，表示使用后端云服务器端口号。  [当pool协议为IP时，monitor_port必须指定为非0值。](tag:hws_eu)  >当绑定的pool开启了端口透传功能时，该字段为必填。
    * name  健康检查名称。
    ...
    * Sets monitorPort
    *
    * @param int|null $monitorPort 健康检查端口号。取值：1-65535，默认为空，表示使用后端云服务器端口号。  [当pool协议为IP时，monitor_port必须指定为非0值。](tag:hws_eu)  >当绑定的pool开启了端口透传功能时，该字段为必填。
    *
    * @return $this
    */
