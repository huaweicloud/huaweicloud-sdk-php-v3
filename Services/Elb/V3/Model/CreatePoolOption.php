...
class CreatePoolOption implements ModelInterface, ArrayAccess
{
    ...
    * slowStart  slowStart
    * memberDeletionProtectionEnable  参数解释：是否开启删除保护。  取值范围：false不开启，true开启，默认false。  > 退场时需要先关闭所有资源的删除保护开关。  [不支持该字段，请勿使用。](tag:hws_eu,g42,hk_g42)  [荷兰region不支持该字段，请勿使用。](tag:dt)
    ...
    *
    * @param bool|null $memberDeletionProtectionEnable 参数解释：是否开启删除保护。  取值范围：false不开启，true开启，默认false。  > 退场时需要先关闭所有资源的删除保护开关。  [不支持该字段，请勿使用。](tag:hws_eu,g42,hk_g42)  [荷兰region不支持该字段，请勿使用。](tag:dt)
    *
    * @return $this
    */
