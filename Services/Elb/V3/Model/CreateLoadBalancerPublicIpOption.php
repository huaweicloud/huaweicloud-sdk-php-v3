...
class CreateLoadBalancerPublicIpOption implements ModelInterface, ArrayAccess
{
    ...
    * ipVersion  **参数解释**：IP版本。  **约束限制**：不涉及  **取值范围**：  - 4：表示IPv4。  - 6：表示IPv6。  **默认取值**：4  [不支持IPv6，请勿设置为6。](tag:dt)
    * networkType  **参数解释**：弹性公网IP的网络类型，默认5_bgp，更多请参考弹性公网ip创建。  [**约束限制**：华南-深圳局点该参数取值只能为5_gray。](tag:hws) [**约束限制**：只支持设置为5_gray。](tag:dt)  **取值范围**：5_bgp - 全动态BGP。  **默认取值**：5_bgp
    * billingInfo  **参数解释**：计费订单信息。  **约束限制**： - 空：按需计费。 [- 非空：包周期计费。格式：order_id:product_id:region_id:project_id](tag:hws)  **取值范围**：不涉及  **默认取值**：不涉及  [不支持该字段，请勿使用。](tag:hws_hk,hws_eu,hws_eu_wb,hws_test,srg,fcs,fcs_vm,dt,ctc,cmcc,tm,sbc,hk_sbc,hk_tm,hk_vdf,ct)
    * description  **参数解释**：弹性公网IP的描述信息。  **约束限制**：不涉及  **取值范围**：不涉及  **默认取值**：不涉及
    * bandwidth  bandwidth
    *
    * @var string[]
    */
    ...
    * Sets ipVersion
    *
    * @param int|null $ipVersion **参数解释**：IP版本。  **约束限制**：不涉及  **取值范围**：  - 4：表示IPv4。  - 6：表示IPv6。  **默认取值**：4  [不支持IPv6，请勿设置为6。](tag:dt)
    *
    * @return $this
    */
    ...
    * Sets networkType
    *
    * @param string $networkType **参数解释**：弹性公网IP的网络类型，默认5_bgp，更多请参考弹性公网ip创建。  [**约束限制**：华南-深圳局点该参数取值只能为5_gray。](tag:hws) [**约束限制**：只支持设置为5_gray。](tag:dt)  **取值范围**：5_bgp - 全动态BGP。  **默认取值**：5_bgp
    *
    * @return $this
    */
    ...
    * Sets billingInfo
    *
    * @param string|null $billingInfo **参数解释**：计费订单信息。  **约束限制**： - 空：按需计费。 [- 非空：包周期计费。格式：order_id:product_id:region_id:project_id](tag:hws)  **取值范围**：不涉及  **默认取值**：不涉及  [不支持该字段，请勿使用。](tag:hws_hk,hws_eu,hws_eu_wb,hws_test,srg,fcs,fcs_vm,dt,ctc,cmcc,tm,sbc,hk_sbc,hk_tm,hk_vdf,ct)
    *
    * @return $this
    */
    ...
    * Sets description
    *
    * @param string|null $description **参数解释**：弹性公网IP的描述信息。  **约束限制**：不涉及  **取值范围**：不涉及  **默认取值**：不涉及
    *
    * @return $this
    */
