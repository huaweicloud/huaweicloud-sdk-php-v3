...
class BatchCreateMembersOption implements ModelInterface, ArrayAccess
{
    ...
    * protocolPort  **参数解释**：后端服务器业务端口。  **约束限制**：网关型LB，即pool协议为IP时，protocol_port必须设置为0。  **取值范围**：不涉及  **默认取值**：不涉及  >在开启端口透传的pool下创建member传该字段不生效，可不传该字段。
    ...
    * Sets protocolPort
    *
    * @param int $protocolPort **参数解释**：后端服务器业务端口。  **约束限制**：网关型LB，即pool协议为IP时，protocol_port必须设置为0。  **取值范围**：不涉及  **默认取值**：不涉及  >在开启端口透传的pool下创建member传该字段不生效，可不传该字段。
    *
    * @return $this
    */
