...
class UpdateHealthMonitorOption implements ModelInterface, ArrayAccess
{
    ...
    * expectedCodes  期望响应状态码。  取值： - 单值：单个返回码，例如200。 - 列表：多个特定返回码，例如200，202。 - 区间：一个返回码区间，例如200-204。  仅支持HTTP/HTTPS/gRPC设置该字段，其他协议设置不会生效。
    ...
    * monitorPort  健康检查端口号。取值：1-65535，不可传入空，但可传入null，表示使用后端云服务器端口号。[当pool协议为IP时，monitor_port必须指定为非0值。](tag:hws_eu)
    * name  健康检查名称。
    ...
    * Sets expectedCodes
    *
    * @param string|null $expectedCodes 期望响应状态码。  取值： - 单值：单个返回码，例如200。 - 列表：多个特定返回码，例如200，202。 - 区间：一个返回码区间，例如200-204。  仅支持HTTP/HTTPS/gRPC设置该字段，其他协议设置不会生效。
    *
    * @return $this
    */
    ...
    * Sets monitorPort
    *
    * @param int|null $monitorPort 健康检查端口号。取值：1-65535，不可传入空，但可传入null，表示使用后端云服务器端口号。[当pool协议为IP时，monitor_port必须指定为非0值。](tag:hws_eu)
    *
    * @return $this
    */
