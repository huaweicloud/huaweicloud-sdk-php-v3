...
class Pool implements ModelInterface, ArrayAccess
{
    ...
    * adminStateUp  参数解释：后端服务器组的管理状态。  [不支持该字段，请勿使用。](tag:dt,dt_test,hcso_dt)
    ...
    * slowStart  slowStart
    * memberDeletionProtectionEnable  参数解释：是否开启误删保护。  取值范围：false不开启，true开启。  > 退场时需要先关闭所有资源的删除保护开关。  [不支持该字段，请勿使用。](tag:hws_eu,g42,hk_g42)  [荷兰region不支持该字段，请勿使用。](tag:dt,dt_test)
    ...
    * protectionReason  参数解释：设置保护的原因。  参数限制：仅当protection_status为consoleProtection时有效。
    * anyPortEnable  参数解释：后端是否开启端口透传。开启后，后端服务器端口与前端监听器端口保持一致。关闭后，请求会转发给后端服务器protocol_port字段指定端口。取值：false不开启，true开启。  约束限制： - 仅QUIC,TCP,UDP的pool支持。
    ...
    * Sets adminStateUp
    *
    * @param bool $adminStateUp 参数解释：后端服务器组的管理状态。  [不支持该字段，请勿使用。](tag:dt,dt_test,hcso_dt)
    *
    * @return $this
    */
    ...
    *
    * @param bool $memberDeletionProtectionEnable 参数解释：是否开启误删保护。  取值范围：false不开启，true开启。  > 退场时需要先关闭所有资源的删除保护开关。  [不支持该字段，请勿使用。](tag:hws_eu,g42,hk_g42)  [荷兰region不支持该字段，请勿使用。](tag:dt,dt_test)
    *
    * @return $this
    */
    ...
    *
    * @param string|null $protectionReason 参数解释：设置保护的原因。  参数限制：仅当protection_status为consoleProtection时有效。
    *
    * @return $this
    */
    ...
    * Sets anyPortEnable
    *
    * @param bool|null $anyPortEnable 参数解释：后端是否开启端口透传。开启后，后端服务器端口与前端监听器端口保持一致。关闭后，请求会转发给后端服务器protocol_port字段指定端口。取值：false不开启，true开启。  约束限制： - 仅QUIC,TCP,UDP的pool支持。
    *
    * @return $this
    */
