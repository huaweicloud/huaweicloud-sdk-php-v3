...
class UpdateRedirectUrlConfig implements ModelInterface, ArrayAccess
{
    ...
    * protocol  **参数解释**：重定向的协议。  **约束限制**：不涉及  **取值范围**： - HTTP - HTTPS - ${protocol}，表示继承原值（即与被转发请求保持一致）。  **默认取值**：不涉及
    ...
    * path  **参数解释**：重定向的路径。  **约束限制**：不涉及  **取值范围**：只能包含英文字母、数字、_~';@^-%#&$.\\*+?,=!:|\\/()\\[\\]{}，且必须以\"/\"开头。  **默认取值**：${path}，表示继承原值（即与被转发请求保持一致）。
    ...
    * Sets protocol
    *
    * @param string|null $protocol **参数解释**：重定向的协议。  **约束限制**：不涉及  **取值范围**： - HTTP - HTTPS - ${protocol}，表示继承原值（即与被转发请求保持一致）。  **默认取值**：不涉及
    *
    * @return $this
    */
    ...
    * Sets path
    *
    * @param string|null $path **参数解释**：重定向的路径。  **约束限制**：不涉及  **取值范围**：只能包含英文字母、数字、_~';@^-%#&$.\\*+?,=!:|\\/()\\[\\]{}，且必须以\"/\"开头。  **默认取值**：${path}，表示继承原值（即与被转发请求保持一致）。
    *
    * @return $this
    */
    ...
    * Sets statusCode
    *
    * @param string|null $statusCode **参数解释**：重定向后的返回码。  **约束限制**：不涉及  **取值范围**： - 301 - 302 - 303 - 307 - 308  **默认取值**：不涉及
    *
    * @return $this
    */
