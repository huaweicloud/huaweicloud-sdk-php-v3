...
class CreateCorsConfig implements ModelInterface, ArrayAccess
{
    ...
    * allowOrigin  **参数解释**：允许的访问来源列表。  **约束限制**： - 若配置了通配符（*），则不能配其他值。 - 不支持IP地址。 - 不能配置重复的值。  **取值范围**：单个列表项的取值 - 通配符（*），表示匹配所有来源。 - 除通配符外，其他值只能是以http://或者https://开头（小写），后面加域名。可以是具体域名或一级泛域名，可以包含端口或不包含端口。例：http://_*.test.abc.example.com:80  **默认取值**：不涉及
    ...
    * Sets allowOrigin
    *
    * @param string[]|null $allowOrigin **参数解释**：允许的访问来源列表。  **约束限制**： - 若配置了通配符（*），则不能配其他值。 - 不支持IP地址。 - 不能配置重复的值。  **取值范围**：单个列表项的取值 - 通配符（*），表示匹配所有来源。 - 除通配符外，其他值只能是以http://或者https://开头（小写），后面加域名。可以是具体域名或一级泛域名，可以包含端口或不包含端口。例：http://_*.test.abc.example.com:80  **默认取值**：不涉及
    *
    * @return $this
    */
