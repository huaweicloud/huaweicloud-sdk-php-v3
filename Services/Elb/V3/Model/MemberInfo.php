...
class MemberInfo implements ModelInterface, ArrayAccess
{
    ...
    * name  **参数解释**：后端服务器名称。注意：该名称并非ECS名称。  **取值范围**：不涉及
    * projectId  **参数解释**：后端服务器所在的项目ID。  **取值范围**：不涉及
    * poolId  **参数解释**：所在后端服务器组ID。  **取值范围**：不涉及  不支持该字段，请勿使用。
    * adminStateUp  **参数解释**：后端服务器的管理状态。虽然创建、更新请求支持该字段，但实际取值决定于后端服务器对应的弹性云服务器是否存在。若存在，该值为true，否则，该值为false。  **取值范围**：不涉及true、false。
    ...
    * protocolPort  **参数解释**：后端服务器业务端口。  **取值范围**：不涉及
    * weight  **参数解释**：后端服务器的权重，请求将根据pool配置的负载均衡算法和后端服务器的权重进行负载分发。 权重值越大，分发的请求越多。权重为0的后端不再接受新的请求。  **取值范围**：0-100
    * address  **参数解释**：后端服务器对应的IP地址。  **取值范围**： - 若subnet_cidr_id为空，表示添加IP类型后端，此时address必须为**私网IPv4**地址。 - 若subnet_cidr_id不为空，表示关联到非IP类型后端，可以是ECS后端服务器、辅助弹性网卡、bms。该IP地址必须在subnet_cidr_id对应的子网网段中，可以是**私网IPv4**或IPv6。  [不支持IPv6，请勿设置为IPv6地址。](tag:dt)
    ...
    * operatingStatus  **参数解释**：后端服务器的健康状态。当status非空时，以status字段中监听器粒度的健康检查状态优先。  **取值范围**： - NO_MONITOR：后端服务器所在的服务器组没有开启健康检查。 - INITIAL：初始化中，表示负载均衡实例配置了健康检查，但查不到数据。 - ONLINE：后端服务器正常。 - OFFLINE：后端服务器关联的ECS服务器不存在或已关机。 - UNKNOWN：未关联LB实例的pool下的member，或者创建后从未关联ECS的云服务器类型member，状态置为UNKNOWN。
    * status  **参数解释**：后端服务器监听器粒度的的健康状态。 若绑定的监听器在该字段中，则以该字段中监听器对应的operating_status为准。 若绑定的监听器不在该字段中，则以外层的operating_status为准。
    * loadbalancerId  **参数解释**：所属负载均衡器ID。  不支持该字段，请勿使用。
    ...
    *
    * @param string $availabilityZone **参数解释**：后端服务器所在的可用区。  **取值范围**：不涉及
    *
    * @return $this
    */
    ...
    * Sets name
    *
    * @param string $name **参数解释**：后端服务器名称。注意：该名称并非ECS名称。  **取值范围**：不涉及
    *
    * @return $this
    */
    ...
    * Sets poolId
    *
    * @param string|null $poolId **参数解释**：所在后端服务器组ID。  **取值范围**：不涉及  不支持该字段，请勿使用。
    *
    * @return $this
    */
    ...
    * Sets adminStateUp
    *
    * @param bool $adminStateUp **参数解释**：后端服务器的管理状态。虽然创建、更新请求支持该字段，但实际取值决定于后端服务器对应的弹性云服务器是否存在。若存在，该值为true，否则，该值为false。  **取值范围**：不涉及true、false。
    *
    * @return $this
    */
    ...
    * Sets protocolPort
    *
    * @param int $protocolPort **参数解释**：后端服务器业务端口。  **取值范围**：不涉及
    *
    * @return $this
    */
    ...
    * Sets weight
    *
    * @param int $weight **参数解释**：后端服务器的权重，请求将根据pool配置的负载均衡算法和后端服务器的权重进行负载分发。 权重值越大，分发的请求越多。权重为0的后端不再接受新的请求。  **取值范围**：0-100
    *
    * @return $this
    */
    ...
    * Sets address
    *
    * @param string $address **参数解释**：后端服务器对应的IP地址。  **取值范围**： - 若subnet_cidr_id为空，表示添加IP类型后端，此时address必须为**私网IPv4**地址。 - 若subnet_cidr_id不为空，表示关联到非IP类型后端，可以是ECS后端服务器、辅助弹性网卡、bms。该IP地址必须在subnet_cidr_id对应的子网网段中，可以是**私网IPv4**或IPv6。  [不支持IPv6，请勿设置为IPv6地址。](tag:dt)
    *
    * @return $this
    */
    ...
    * Sets deviceOwner
    *
    * @param string|null $deviceOwner **参数解释**：设备所有者。  **取值范围**：不涉及 - 空，表示后端服务器未关联到ECS。 - compute:{az_name}，表示关联到ECS，其中{az_name}表示ECS所在可用区名。 - compute:subeni，表示辅助弹性网卡。  不支持该字段，请勿使用。
    *
    * @return $this
    */
    ...
    * Sets deviceId
    *
    * @param string|null $deviceId **参数解释**：关联的ECS ID，为空表示后端服务器未关联到ECS。  **取值范围**：不涉及  不支持该字段，请勿使用。
    *
    * @return $this
    */
    ...
    *
    * @param string $operatingStatus **参数解释**：后端服务器的健康状态。当status非空时，以status字段中监听器粒度的健康检查状态优先。  **取值范围**： - NO_MONITOR：后端服务器所在的服务器组没有开启健康检查。 - INITIAL：初始化中，表示负载均衡实例配置了健康检查，但查不到数据。 - ONLINE：后端服务器正常。 - OFFLINE：后端服务器关联的ECS服务器不存在或已关机。 - UNKNOWN：未关联LB实例的pool下的member，或者创建后从未关联ECS的云服务器类型member，状态置为UNKNOWN。
    *
    * @return $this
    */
    ...
    * Sets status
    *
    * @param \HuaweiCloud\SDK\Elb\V3\Model\MemberStatus[] $status **参数解释**：后端服务器监听器粒度的的健康状态。 若绑定的监听器在该字段中，则以该字段中监听器对应的operating_status为准。 若绑定的监听器不在该字段中，则以外层的operating_status为准。
    *
    * @return $this
    */
    ...
    * Sets loadbalancerId
    *
    * @param string|null $loadbalancerId **参数解释**：所属负载均衡器ID。  不支持该字段，请勿使用。
    *
    * @return $this
    */
    ...
    * Sets loadbalancers
    *
    * @param \HuaweiCloud\SDK\Elb\V3\Model\ResourceID[]|null $loadbalancers **参数解释**：后端服务器关联的负载均衡器ID列表。  不支持该字段，请勿使用。
    *
    * @return $this
    */
    ...
    * Sets instanceId
    *
    * @param string|null $instanceId **参数解释**：member关联的实例ID。空表示member关联的实例为非真实设备 （如：跨VPC场景）。  **取值范围**：不涉及
    *
    * @return $this
    */
