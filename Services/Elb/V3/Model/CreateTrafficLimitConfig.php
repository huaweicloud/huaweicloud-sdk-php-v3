...
class CreateTrafficLimitConfig implements ModelInterface, ArrayAccess
{
    ...
    * perSourceIpQps  对转发策略单源进行限速。 quic监听器下转发策略不支持配置单源限速。指定该字段时，赋值可以为0或者为null。 取值： 0-100000s。0表示不限速，如果qps不为0，per_source_ip_qps需要小于qps。
    ...
    * Sets perSourceIpQps
    *
    * @param int|null $perSourceIpQps 对转发策略单源进行限速。 quic监听器下转发策略不支持配置单源限速。指定该字段时，赋值可以为0或者为null。 取值： 0-100000s。0表示不限速，如果qps不为0，per_source_ip_qps需要小于qps。
    *
    * @return $this
    */
