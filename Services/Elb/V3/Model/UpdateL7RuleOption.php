...
class UpdateL7RuleOption implements ModelInterface, ArrayAccess
{
    ...
    * invert  **参数解释**：是否反向匹配。  **约束限制**：不涉及  **取值范围**：true、false。  **默认取值**：不涉及  不支持该字段，请勿使用。
    ...
    * Sets compareType
    *
    * @param string|null $compareType **参数解释**：转发匹配方式。  **约束限制**： - type为HOST_NAME时仅支持EQUAL_TO，支持通配符*。 - type为PATH时可以为REGEX，STARTS_WITH，EQUAL_TO。 - type为METHOD、SOURCE_IP时，仅支持EQUAL_TO。 - type为HEADER、QUERY_STRING，仅支持EQUAL_TO，支持通配符*、？。  **取值范围**： - EQUAL_TO 表示精确匹配。 - REGEX 表示正则匹配。 - STARTS_WITH 表示前缀匹配。  **默认取值**：不涉及
    *
    * @return $this
    */
    ...
    * Sets invert
    *
    * @param bool|null $invert **参数解释**：是否反向匹配。  **约束限制**：不涉及  **取值范围**：true、false。  **默认取值**：不涉及  不支持该字段，请勿使用。
    *
    * @return $this
    */
    ...
    * Sets conditions
    *
    * @param \HuaweiCloud\SDK\Elb\V3\Model\UpdateRuleCondition[]|null $conditions **参数解释**：转发规则的匹配条件。  **约束限制**： - 当监听器的高级转发策略功能（enhance_l7policy_enable）开启后才会生效。 - 若转发规则配置了conditions，字段key、字段value的值无意义。 - 同一个rule内的conditions列表中所有key必须相同，value不允许重复。  **取值范围**：不涉及  **默认取值**：不涉及  [不支持该字段，请勿使用。](tag:hcso_dt)  [荷兰region不支持该字段，请勿使用。](tag:dt)
    *
    * @return $this
    */
