...
class CreateLoadBalancerOption implements ModelInterface, ArrayAccess
{
    ...
    * ipv6VipAddress  **参数解释**：负载均衡器实例的IPv6地址。  **约束限制**：  - 必须属于ipv6_vip_virsubnet_id子网中地址。  - elb_virsubnet_ids中的后端子网必须支持双栈。  [- 网关型LB不支持指定ipv6_vip_address。](tag:hws_eu)  **取值范围**：不涉及  **默认取值**：不涉及  [不支持IPv6，请勿使用。](tag:dt)
    *
    * @var string[]
    */
    ...
    * Sets ipv6VipAddress
    *
    * @param string|null $ipv6VipAddress **参数解释**：负载均衡器实例的IPv6地址。  **约束限制**：  - 必须属于ipv6_vip_virsubnet_id子网中地址。  - elb_virsubnet_ids中的后端子网必须支持双栈。  [- 网关型LB不支持指定ipv6_vip_address。](tag:hws_eu)  **取值范围**：不涉及  **默认取值**：不涉及  [不支持IPv6，请勿使用。](tag:dt)
    *
    * @return $this
    */
