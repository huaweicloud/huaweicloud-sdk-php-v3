...
class UpdateAzAffinity implements ModelInterface, ArrayAccess
{
    ...
    * enable  **参数解释**：后端服务器组可用区亲和开关。  **约束限制**： - 当后端服务器组中有未设置availability_zone属性的IP类型后端服务器时无法开启可用区亲和。 - 当后端服务器绑定TLS监听器时无法开启可用区亲和。 - 仅IP、UDP、TCP类型的后端服务器组支持开启可用区亲和。 - 当开启可用区亲和后，原本的pool_health配置失效。  **取值范围**：true或false，true表示开启，false表示关闭。  **默认取值**：不涉及
    ...
    * Sets enable
    *
    * @param bool|null $enable **参数解释**：后端服务器组可用区亲和开关。  **约束限制**： - 当后端服务器组中有未设置availability_zone属性的IP类型后端服务器时无法开启可用区亲和。 - 当后端服务器绑定TLS监听器时无法开启可用区亲和。 - 仅IP、UDP、TCP类型的后端服务器组支持开启可用区亲和。 - 当开启可用区亲和后，原本的pool_health配置失效。  **取值范围**：true或false，true表示开启，false表示关闭。  **默认取值**：不涉及
    *
    * @return $this
    */
