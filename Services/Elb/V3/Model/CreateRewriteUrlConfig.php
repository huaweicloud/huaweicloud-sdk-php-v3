...
class CreateRewriteUrlConfig implements ModelInterface, ArrayAccess
{
    ...
    * path  参数解释：重定向的请求路径。其中$1-$9会匹配请求url通配符星号(*)，当正则匹配分组小于指定数字，则$指定数字结果为空。$后面跟字母，匹配结果均为空，直到下一个特殊字符出现，例如$abc#123，则匹配结果为#123；$后面跟特殊字符则直接输出特殊字符，例如$#匹配结果为$#。  取值范围：英文字母、数字、_~';@^-%#&$.+?,=!:|/()，且必须以\"/\"开头。  默认值${path}表示继承原值（即与被重写请求保持一致）。
    ...
    * Sets path
    *
    * @param string|null $path 参数解释：重定向的请求路径。其中$1-$9会匹配请求url通配符星号(*)，当正则匹配分组小于指定数字，则$指定数字结果为空。$后面跟字母，匹配结果均为空，直到下一个特殊字符出现，例如$abc#123，则匹配结果为#123；$后面跟特殊字符则直接输出特殊字符，例如$#匹配结果为$#。  取值范围：英文字母、数字、_~';@^-%#&$.+?,=!:|/()，且必须以\"/\"开头。  默认值${path}表示继承原值（即与被重写请求保持一致）。
    *
    * @return $this
    */
