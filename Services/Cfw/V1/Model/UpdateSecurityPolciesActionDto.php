...
class UpdateSecurityPolciesActionDto implements ModelInterface, ArrayAccess
{
    ...
    * action  **参数解释**： 规则动作 **约束限制**： 不涉及 **取值范围**： enable表示允许通行（permit），disable表示拒绝通行（deny） **默认取值**： 不涉及
    * ruleIds  **参数解释**： 规则ID列表，规则ID可通过[查询防护规则接口](ListAclRules.xml)查询获得，通过返回值中的data.records.rule_id（.表示各对象之间层级的区分）获得。 **约束限制**： 不涉及 **取值范围**： 不涉及 **默认取值**： 不涉及
    *
    * @var string[]
    */
    ...
    * Sets action
    *
    * @param string $action **参数解释**： 规则动作 **约束限制**： 不涉及 **取值范围**： enable表示允许通行（permit），disable表示拒绝通行（deny） **默认取值**： 不涉及
    *
    * @return $this
    */
    ...
    * Sets ruleIds
    *
    * @param string[] $ruleIds **参数解释**： 规则ID列表，规则ID可通过[查询防护规则接口](ListAclRules.xml)查询获得，通过返回值中的data.records.rule_id（.表示各对象之间层级的区分）获得。 **约束限制**： 不涉及 **取值范围**： 不涉及 **默认取值**： 不涉及
    *
    * @return $this
    */
