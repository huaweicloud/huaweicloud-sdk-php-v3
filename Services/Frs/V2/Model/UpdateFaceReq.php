...
class UpdateFaceReq implements ModelInterface, ArrayAccess
{
    ...
    * externalFields  Json字符串不校验重复性，自定义字段的key值长度范围为[1,36]，string类型的value长度范围为[1,256]，具体参见[自定义字段](zh-cn_topic_0130807044.xml)。 这里是待修改的参数，external_image_id和external_fields至少选一个。
    ...
    * Sets externalFields
    *
    * @param object|null $externalFields Json字符串不校验重复性，自定义字段的key值长度范围为[1,36]，string类型的value长度范围为[1,256]，具体参见[自定义字段](zh-cn_topic_0130807044.xml)。 这里是待修改的参数，external_image_id和external_fields至少选一个。
    *
    * @return $this
    */
